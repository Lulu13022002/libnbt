...
static void nbt__get_string(nbt__read_stream_t* stream, nbt_tag_t* tag) {
...
static nbt_tag_t* nbt__parse(nbt__read_stream_t* stream, int parse_name, nbt_tag_type_t override_type) {
  ...
  switch (tag->type) {
    ...
    case NBT_TYPE_SHORT_ARRAY: {
      tag->tag_short_array.size = nbt__get_int32(stream);
      tag->tag_short_array.value = (int16_t*)NBT_MALLOC(tag->tag_short_array.size * sizeof(int16_t));
      for (size_t i = 0; i < tag->tag_short_array.size; i++) {
        tag->tag_short_array.value[i] = nbt__get_int16(stream);
      }
      break;
    }
    ...
    case NBT_TYPE_STRING_ARRAY: {
      tag->tag_string_array.size = nbt__get_int32(stream);
      tag->tag_string_array.value = (nbt_tag_t**)NBT_MALLOC(tag->tag_string_array.size * sizeof(nbt_tag_t*));
      for (size_t i = 0; i < tag->tag_string_array.size; i++) {
          tag->tag_string_array.value[i] = nbt__parse(stream, 0, NBT_TYPE_STRING);
      }
      break;
    }
    default: {
...
DllExport nbt_tag_t* nbt_parse(nbt_reader_t reader, int parse_flags) {
...
void nbt__put_string(nbt__write_stream_t* stream, nbt_tag_t* tag) {
...
void nbt__write_tag(nbt__write_stream_t* stream, nbt_tag_t* tag, int write_name, int write_type) {
  ...
  switch (tag->type) {
    ...
    case NBT_TYPE_SHORT_ARRAY: {
      nbt__put_int32(stream, tag->tag_short_array.size);
      for (size_t i = 0; i < tag->tag_short_array.size; i++) {
        nbt__put_int16(stream, tag->tag_short_array.value[i]);
      }
      break;
    }
    case NBT_TYPE_FLOAT_ARRAY: {
    ...
    case NBT_TYPE_STRING_ARRAY: {
      nbt__put_int32(stream, tag->tag_string_array.size);
      for (size_t i = 0; i < tag->tag_string_array.size; i++) {
        nbt__put_string(stream, tag->tag_string_array.value[i]);
      }
      break;
    }
    default: {
...
DllExport nbt_tag_t* nbt_new_tag_byte(int8_t value) {
...
DllExport nbt_tag_t* nbt_new_tag_short(int16_t value) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_int(int32_t value) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_long(int64_t value) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_float(float value) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_double(double value) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_byte_array(int8_t* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_string(const char* value) {
  return nbt_new_tag_string0(value, strlen(value));
}

DllExport nbt_tag_t* nbt_new_tag_string0(const char* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_list(nbt_tag_type_t type) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_compound(void) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_int_array(int32_t* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_long_array(int64_t* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();
...
DllExport nbt_tag_t* nbt_new_tag_short_array(int16_t* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();

  tag->type = (nbt_tag_type_t)NBT_TYPE_SHORT_ARRAY;
  tag->tag_short_array.size = size;
  tag->tag_short_array.value = (int16_t*)NBT_MALLOC(size * sizeof(int16_t));

  NBT_MEMCPY(tag->tag_short_array.value, value, size * sizeof(int16_t));

  return tag;
}

DllExport nbt_tag_t* nbt_new_tag_float_array(float* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();

  tag->type = (nbt_tag_type_t)NBT_TYPE_FLOAT_ARRAY;
...
DllExport nbt_tag_t* nbt_new_tag_double_array(double* value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();

  tag->type = (nbt_tag_type_t)NBT_TYPE_DOUBLE_ARRAY;
  tag->tag_double_array.size = size;
  tag->tag_double_array.value = (double*)NBT_MALLOC(size * sizeof(double));

  NBT_MEMCPY(tag->tag_double_array.value, value, size * sizeof(double));

  return tag;
}

DllExport nbt_tag_t* nbt_new_tag_string_array(char** value, size_t size) {
  nbt_tag_t* tag = nbt__new_tag_base();

  tag->type = (nbt_tag_type_t)NBT_TYPE_STRING_ARRAY;
  tag->tag_string_array.size = size;
  tag->tag_string_array.value = (nbt_tag_t**)NBT_MALLOC(size * sizeof(nbt_tag_t*));

  for(size_t i = 0; i < size; i++) {
    tag->tag_string_array.value[i] = nbt_new_tag_string(value[i]);
  }

  return tag;
}

DllExport void nbt_set_tag_name(nbt_tag_t* tag, const char* name) {
  nbt_set_tag_name0(tag, name, strlen(name));
}

DllExport void nbt_set_tag_name0(nbt_tag_t* tag, const char* name, size_t size) {
  if (tag->name) {
    NBT_FREE(tag->name);
  }
  tag->name_size = size;
...
DllExport void nbt_tag_list_append(nbt_tag_t* list, nbt_tag_t* value) {
...
DllExport nbt_tag_t* nbt_tag_list_get(nbt_tag_t* tag, size_t index) {
  return tag->tag_list.value[index];
}

DllExport void nbt_tag_compound_append(nbt_tag_t* compound, nbt_tag_t* value) {
...
DllExport nbt_tag_t* nbt_tag_compound_get(nbt_tag_t* tag, const char* key) {
  for (size_t i = 0; i < tag->tag_compound.size; i++) {
...
DllExport void nbt_free_tag(nbt_tag_t* tag) {
  switch (tag->type) {
    case NBT_TYPE_BYTE_ARRAY: {
    ...
    case NBT_TYPE_SHORT_ARRAY: {
      NBT_FREE(tag->tag_short_array.value);
      break;
    }
    case NBT_TYPE_FLOAT_ARRAY: {
    ...
    case NBT_TYPE_STRING_ARRAY: {
      for (size_t i = 0; i < tag->tag_string_array.size; i++) {
        nbt_free_tag(tag->tag_string_array.value[i]);
      }
      NBT_FREE(tag->tag_string_array.value);
      break;
    }
    default: {
      break;
